...
public class DynamicJndiContextFactoryBuilder implements InitialContextFactoryBuilder {
    ...
    public DynamicJndiContextFactoryBuilder(DataSource dataSource,
                                            DefaultCacheManager cacheManager,
                                            ExecutorService executorService) {
        fixedInitialContext = createFixedInitialContext(dataSource, cacheManager, executorService);
    }

    protected InitialContext createFixedInitialContext(DataSource dataSource,
                                                       DefaultCacheManager cacheManager,
                                                       ExecutorService executorService) {
     ...
     * <p>
     * If the lookup environment is empty, we return a JndiContextFactory that returns a InitialContext
     * which supports a lookups against a fixed set of Spring beans.
     * If the environment is not empty, we try to use the provided java.naming.factory.initial
     * classname to create a JndiContextFactory and delegate further lookups to this instance.
     * Otherwise we simply return {@literal null}.
     *
     * @param environment
     * @return
     */
    ...
    public InitialContextFactory createInitialContextFactory(Hashtable<?, ?> environment) {
        ...
        if (factoryClassName != null) {
            try {
                ...
                initialContextFactory = BeanUtils.instantiateClass(factoryClass, InitialContextFactory.class);
